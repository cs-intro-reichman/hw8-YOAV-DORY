 ...
 public class User {
    ...
    public boolean follows(String name) {
        for(int i = 0; i < follows.length; i++){
            if (follows[i]!=null) if (follows[i].equals(name)) return true;
        }
        return false;
    }
    ...
    public boolean addFollowee(String name) {
        for (int i = 0; i < fCount; i++){
            if(follows[i]!= null && follows[i].equals(name)) return false;
        }
        int i = 0;
        while (i < maxfCount && follows[i] != null) {
            i++;
        }
        if (i == maxfCount) return false;  // אין מקום
        follows[i] = name;
        fCount++;
        return true;
    ...
    public boolean isFriendOf(User other) {
        boolean ifollowU = false;
        boolean ufollowMe = false;
        for(int i = 0; i < fCount; i++){
        ...
        for(int i = 0; i < other.getfCount(); i++){
            if (other.getfFollows()[i] != null && other.getfFollows()[i].equals(this.name) ) {
                ufollowMe = true;
                break;
            }
        }
